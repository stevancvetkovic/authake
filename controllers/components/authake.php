...
/*
	This component is the heart of the system. It manages the user login 
	and checks the URL according to groups and rules. The job is done in 
	the AppController::beforeFilter() callback (/app/app_controller.php). 

	Options for the component are in the core config file 
	(/app/config/core.php).
*/
...
{  
	...
	function beforeFilter(&$controller) 
	{
		...
		{ // check for permissions
			if ($this->isLogged())
			{ // if denied & logged, write a message
			...
			{ // if denied & not loggued, propose to log in
				$this->setPreviousUrl($path);
	...
	function getRules($group_ids = null) {
		...
		if($force_reload || is_array($group_ids) || ($cacheRules = $this->Session->read('Authake.cacheRules')) === null)
		{
			...
			if ($group_ids === null) 
			{ // cache only if groups of user used
