...
{        
	...
	{
		$img = parent::image("icons/{$icon}.png", array('title' => $title));
		return parent::link($img, $url, $htmlAttributes, $confirmMessage, false);
	}
    
	function iconallowdeny($what)
	{
		if ($what == 'Allow')
			echo $this->image("icons/accept.png", array('title' => __('Allow', true)));
		else
			echo $this->image("icons/delete.png", array('title' => __('Deny', true)));
	}   
}
?>
